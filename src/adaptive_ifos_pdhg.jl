...
function define_norms(
    ...
    return (1 / step_size * primal_weight) ./ primal_hyperparam, (1 / step_size / primal_weight) ./ dual_hyperparam
end
...
function update_primal_hyper_state_kernel!(
    ...
    if tx <= num_variables
        @inbounds begin
            primal_grad = primal_step_size * (objective_vector[tx] - current_dual_product[tx] - delta_dual_product[tx]) * (objective_vector[tx] - current_dual_product[tx]) / (primal_norm_squared + eps())
            primal_hypermomentum[tx] += primal_grad * primal_grad
            primal_hypergradient[tx] += learning_rate * primal_grad / sqrt(primal_hypermomentum[tx] + eps())
        end
    end
    return 
end
...
function update_dual_hyper_state_kernel!(
    ...
    if tx <= num_constraints
        @inbounds begin
            dual_grad = dual_step_size * (right_hand_side[tx] - current_primal_product[tx] - delta_primal_product[tx]) * (right_hand_side[tx] - (1 + extrapolation_coefficient) * delta_primal_product[tx] - extrapolation_coefficient * current_primal_product[tx]) / (dual_norm_squared + eps())
            dual_hypermomentum[tx] += dual_grad * dual_grad
            dual_hypergradient[tx] += learning_rate * dual_grad / sqrt(dual_hypermomentum[tx] + eps())
        end
    end
    return 
end
...
function update_hyper_param!(
    hyper_state::CuHyperState,
)
    hyper_state.primal_hyperparam .+= hyper_state.primal_hypergradient
    hyper_state.dual_hyperparam .+= hyper_state.dual_hypergradient

    hyper_state.primal_hypergradient .= 0.0
    hyper_state.dual_hypergradient .= 0.0
    """
    TODO: check the effect of clearing momentum
    """
...
function optimize(
    ...
    d_scaled_problem = scaledqp_cpu_to_gpu(d_scaled_problem)
    ...
    while true
        ...
        if mod(iteration - 1, termination_evaluation_frequency) == 0 ||
            ...
            if params.online_scaling
                update_hyper_param!(hyper_state)
            end
